...
sap.m.TileContainer.prototype._applyDimension = function() {
	...
	jQuery.sap.byId(i + '-scrl').css({
		width : d.outerwidth + 'px',
		height : (d.outerheight - p) + 'px'
	});
	c.css({
		height : (d.outerheight - p) + 'px',
		visibility : 'visible'
	...
	jQuery.sap.byId(i + '-blind').css({
		top : (a.top + o) + 'px',
		left : (a.left + o) + 'px',
		width : (c.outerWidth() - o) + 'px',
		height : (b - o) + 'px'
	});
	jQuery.sap.byId(i + '-rightedge').css({
		top : (t.top + o) + 'px',
		right : o + 'px',
		height : (b - o) + 'px'
	});
	jQuery.sap.byId(i + '-leftedge').css({
		top : (t.top + o) + 'px',
		left : (t.left + o) + 'px',
		height : (b - o) + 'px'
	});
...
sap.m.TileContainer.prototype._initTouchSession = function(e) {
	if (e.type == 'touchstart') {
		...
		this._oTouchSession = {
			...
			fDiffX : 0,
			fDiffY : 0,
			oControl : e.srcControl,
			iOffsetX : t.pageX - e.target.offsetLeft
		};
	} else {
		this._oTouchSession = {
			dStartTime : new Date(),
			...
			fDiffY : 0,
			oControl : e.srcControl,
			iOffsetX : e.pageX - e.target.offsetLeft
		};
	}
};
sap.m.TileContainer.prototype._initDragSession = function(e) {
	...
	if (e.type == 'touchstart') {
		this._oDragSession = {
			oTile : e.srcControl,
			oTileElement : e.srcControl.$()[0],
			iOffsetLeft : e.targetTouches[0].pageX - e.srcControl._posX + this._iScrollLeft,
			iOffsetTop : e.targetTouches[0].pageY - e.srcControl._posY,
			iIndex : i,
			iOldIndex : i,
			iDiffX : e.targetTouches[0].pageX,
			iDiffY : e.targetTouches[0].pageY
		};
	} else {
		this._oDragSession = {
			oTile : e.srcControl,
			oTileElement : e.srcControl.$()[0],
			iOffsetLeft : e.pageX - e.srcControl._posX + this._iScrollLeft,
			iOffsetTop : e.pageY - e.srcControl._posY,
			iIndex : i,
			iOldIndex : i,
			iDiffX : e.pageX,
			iDiffY : e.pageY
